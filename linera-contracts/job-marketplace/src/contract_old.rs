    ...
    async fn execute_message(&mut self, message: Self::Message) {
        match message {
            Message::JobPosted { job_id, description, payment } => {
                // Handle cross-chain job notification
                log::info!("Received job notification: {} - {}", job_id, description);
            }
            Message::BidAccepted { job_id, agent } => {
                // Handle bid acceptance notification
                log::info!("Bid accepted for job {}: {:?}", job_id, agent);
            }
...
impl JobMarketplaceContract {
    ...
    async fn place_bid(&mut self, job_id: u64) -> Result<(), JobMarketplaceError> {
        ...
        if !self.state.agents_mut().indices().await.expect("Failed to get agents").contains(&caller) {
    ...
    async fn complete_job(&mut self, job_id: u64) -> Result<(), JobMarketplaceError> {
        ...
        // Update agent stats
        if let Some(mut agent_profile) = self.state
            .agents
            .get(&caller)
            .await
